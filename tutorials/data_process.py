...
# Define pandas UDF for processing
# @pandas_udf(output_schema)
def process_minibatch(minibatch: pd.DataFrame) -> pd.DataFrame:
    return split_value_weight(minibatch)
...
def generate_output_schema(columns):
    ...
    for column in columns:
        if column == 'label':
            continue
        schema_fields.append(StructField(f'{column}_value', ArrayType(StringType()), True))
...
def main(args):
    ...
    with spark_session:
        ...
        processed_df.write.mode('overwrite').format('orc').save('/home/ubuntu/data/processed_orcs')

        time_cost = start_time - time.time()
